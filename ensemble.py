...
import pandas as pd
# import numpy as np
from decisions import full_ensemble, perc_ensemble
...
def ensemble(numWalks, perc, type, numDel):
    """
    This function does the evaluation of the results. Based on the win / loss formulas from the paper
    :param numWalks: how many forward walks there are
    :param perc: the % of agreement
    :param type: the type of ensemble to look at (test or valid)
    :param numDel: how many iterations to ignore
    :return:
    """
    dollSum = 0
    rewSum = 0
    ...
    for j in range(0, numWalks):
        df = pd.read_csv("../Output/ensemble/results/walk"+str(j)+"ensemble_"+type+".csv", index_col='Date')
        ...
        values.append([
            ...
            str(round(doll, 2)),
            str(round(cov/num, 2)),
            (str(round(pos/cov, 2)) if (cov > 0) else "")
        ])
